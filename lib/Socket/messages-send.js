...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, privateParticipant, privateParticipants, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                const targetParticipant = privateParticipant || privateParticipants;
                for (const device of devices) {
                    const deviceJid = device.jid;
                    const hasKey = !!senderKeyMap[deviceJid];
                    const isPrivateTarget = !!targetParticipant && (
                        Array.isArray(targetParticipant)
                            ? targetParticipant.some(p => areJidsSameUser(deviceJid, p))
                            : areJidsSameUser(deviceJid, targetParticipant)
                    );
                    if ((!hasKey || !!participant) &&
                        (!targetParticipant || isPrivateTarget) &&
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' &&
                ...
                await relayMessage(jid, fullMsg.message, {
                    ...
                    additionalNodes,
                    privateParticipant: options.privateParticipant,
                    privateParticipants: options.privateParticipants
                });
