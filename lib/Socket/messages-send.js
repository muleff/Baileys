...
export const makeMessagesSocket = (config) => {
    ...
    /**
     * Set of tctoken storage JIDs with a fire-and-forget
     * `issuePrivacyTokens` IQ in flight.
     */
    ...
    const getUSyncDevices = async (
            ...
            if (useCache) {
                const devices =
                    ...
                    (
                        userDevicesCache.mget
                            ? undefined
                            : await userDevicesCache.get(
                                user
                            )
                    );

                if (devices) {
                    const devicesWithJid =
                        devices.map(d => ({
                            ...d,
                            jid: jidEncode(
                                d.user,
        ...
        if (result) {
            ...
            if (lidResults.length > 0) {
                logger.trace(
                    'Storing LID maps from device call'
                );
            ...
            for (const [
                ...
                const isLidUserResult =
                    requestedLidUsers.has(
                        user
                    );
            ...
            await devicesMutex.mutex(
                async () => {
                    if (
                        userDevicesCache.mset
                    ) {
    ...
    const assertSessions = async (
        ...
        if (jidsRequiringFetch.length) {
            ...
            await parseAndInjectE2ESessions(
                result,
                signalRepository
            );
    ...
    const sendPeerDataOperationMessage =
        async (pdoMessage) => {
    ...
    const createParticipantNodes =
        async (
            ...
            let shouldIncludeDeviceIdentity =
                false;
            ...
            async () => {
                        ...
                        additionalAttributes =
                            {
                                ...additionalAttributes,
                                expiration:
                                    groupData.ephemeralDuration.toString()
                            };
                    }
                ...
                if (isRetryResend) {
                    ...
                    if (
                        ...
                        if (
                            ...
                            try {
                                const skdm =
                                    await signalRepository.getSenderKeyDistributionMessage(
                                        {
                                            group:
                                                destinationJid,
                                            meId:
                ...
                if (
                    ...
                    try {
                        ...
                        const reportingNode =
                            await getMessageReportingToken(
                                encoded,
                                reportingMessage,
                                reportingKey
                            );
                ...
                if (
                    ...
                    !inFlightTcTokenIssuance.has(
                        tcTokenJid
                    )
                ) {
                    inFlightTcTokenIssuance.add(
                        tcTokenJid
                    );
                    ...
                    resolveIssuanceJid(
                        ...
                        .finally(() => {
                            inFlightTcTokenIssuance.delete(
                                tcTokenJid
                            );
                        });
                }
    ...
    const issuePrivacyTokens = async (
        jids,
        timestamp
    ) => {
        const t =
    ...
    const sendPrivateGroupMessage =
        async (
            ...
            if (!isJidGroup(jid)) {
                throw new Boom(
                    'sendPrivateGroupMessage can only be used with group JIDs',
                    {
                        statusCode: 400
                    }
                );
            ...
            if (
                ...
                throw new Boom(
                    'sendPrivateGroupMessage requires at least one target JID',
                    {
                        statusCode: 400
                    }
                );
            ...
            for (
                ...
                if (
                    ...
                    if (!mapped) {
                        throw new Boom(
                            `Could not resolve LID for private group target ${rawTarget}`,
                            {
                                statusCode:
                                    404
                            }
                    ...
                    if (!mapped) {
                        throw new Boom(
                            `Could not resolve PN for private group target ${rawTarget}`,
                            {
                                statusCode:
                                    404
                            }
            ...
            if (
                ...
                throw new Boom(
                    'No target devices were found for the private group message',
                    {
                        statusCode: 404
                    }
                );
            ...
            await authState.keys.transaction(
                async () => {
                    ...
                    if (
                        ...
                        throw new Boom(
                            `Invalid Signal sender address: ${senderAddress}`,
                            {
                                statusCode:
                                    500
                            }
                    ...
                    if (
                        ...
                        throw new Boom(
                            'Per-jid patching is not supported for private group messages'
                        );
                    }
                    ...
                    if (
                        !deliveredDeviceJids.length
                    ) {
                        throw new Boom(
                            'Failed to distribute the private sender key to the target devices',
                            {
                                statusCode:
                                    500
                            }
                    ...
                    for (
                        const deviceJid of deliveredDeviceJids
                    ) {
                        senderKeyMap[
                            deviceJid
                        ] = true;
        ...
        sendMessage:
            async (
                ...
                if (
                    ...
                    const value =
                        typeof disappearingMessagesInChat ===
                        'boolean'
                            ? disappearingMessagesInChat
                    ...
                    await relayMessage(
                        ...
                        fullMsg.message,
                        {
                            ...
                            privateParticipant:
                                options.privateParticipant,

                            privateParticipants:
                                options.privateParticipants
                        }
                    );
